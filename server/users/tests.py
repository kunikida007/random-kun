...
class SigupTest(TestCase):
    def setUp(self):
        self.index_url = reverse("users:signup")
    ...
    def test_post_fails_by_existed_user(self):
        ...
        response = self.client.post(
            self.index_url,
            {"email": "email@email.com", "username": "username1", "password1": "yy660818", "password2": "yy660818"},
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "users/signup.html")
    ...
    def test_failed_with_lack_password(self):
        ...
        self.assertTemplateUsed(response, "users/signup.html")
        self.assertIn(
            "このパスワードは短すぎます。最低 8 文字以上必要です。", response.context["form"].errors["password2"]
        )
...
class LoginTest(TestCase):
    def test_with_not_existed_user(self):
        form = AuthenticationForm(
            data={"username": "notexistuser", "password": "password_b"}
        )
    ...
    def test_with_wrong_password(self):
        User.objects.create_user("username1", "", "yy660818")
        data = {"username": "username1", "password": "yy11111"}
    ...
    def test_with_correct_user(self):
        User.objects.create_user("username1", "", "yy660818")
        data = {"username": "username1", "password": "yy660818"}
        response = self.client.post(reverse("user:login"), data=data)
        self.assertEqual(User.objects.filter(username="username1").count(), 1)
